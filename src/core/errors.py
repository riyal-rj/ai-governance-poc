...
class AuthRequiredError(AppError):
    def __init__(self, message: str = "Authentication is required") -> None:
        super().__init__(code=ErrorCode.AUTHENTICATION_REQUIRED, message=message)
...
class ResourceConflictError(AppError):
    def __init__(self, message: str, *, details: Mapping[str, Any] | None = None) -> None:
        super().__init__(code=ErrorCode.RESOURCE_CONFLICT, message=message, details=details)
...
class StartupError(AppError):
    def __init__(self, unavailable_dependencies: list[str]) -> None:
        super().__init__(
            code=ErrorCode.STARTUP_FAILED,
            message="Application startup requirements were not satisfied",
            details={"unavailable_dependencies": sorted(unavailable_dependencies)},
            retryable=True,
        )

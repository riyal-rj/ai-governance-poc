...
async def liveness(container: ContainerDependency) -> LivenessResponse:
    ...
    does not repair PostgreSQL or OPA.
    """

    return LivenessResponse(
        service=container.settings.service_name, version=container.settings.service_version
    )


@router.get("/ready", response_model=ReadinessResponse, summary="Dependency readiness")
async def readiness(response: Response, service: ReadinessDependency) -> ReadinessResponse:

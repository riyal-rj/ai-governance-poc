"""Process-scoped asynchronous PostgreSQL connection pool."""
...
class PostgresDatabase:
    ...
    Repositories added in Phase 2 receive this object through dependency
    ...
    async def connect(self) -> None:
        """Open the pool once; fail if PostgreSQL cannot be reached in time."""
        ...
        try:
            ...
            logger.exception(
                ...
                extra={"event": "postgres_connection_failed", "dependency": "postgres"},
            )
    ...
    async def ping(self) -> None:
        """Execute the smallest meaningful round trip through the pool."""
    ...
    async def aclose(self) -> None:
        """Gracefully drain the pool with a bounded shutdown timeout."""

        pool, self._pool = self._pool, None
        if pool is None:
            return
        try:
            await asyncio.wait_for(pool.close(), timeout=self._settings.close_timeout_seconds)
        except TimeoutError:
            logger.error(
                "postgres pool did not close before deadline ; terminating",
                extra={"event": "postgres_close_timeout", "dependency": "postgres"},
            )
            pool.terminate()

"""Low-cardinality Prometheus metrics owned by one application instance."""
...
class Metrics:
    ...
    def __init__(self, *, service_name: str, service_version: str, environment: str) -> None:
        ...
        build = Info("finassist_build", "Build and deployment identity", registry=self.registry)

        build.info(
            {"service": str(service_name), "version": service_version, "environment": environment}
        )
    ...
    def observe_readiness(self, report: ReadinessReport) -> None:
        ...
        for component in report.components:
            self.dependency_up.labels(
                component=component.component, critical=str(component.critical).lower()
